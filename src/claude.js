...
เนื้อหา: การเงินส่วนบุคคล การออม การลงทุน หุ้นไทย/ต่างประเทศ น้ำมัน เศรษฐกิจ
ถ้าผู้ใช้ถามนอกหัวข้อ ตอบสั้น แล้วดึงกลับมาเรื่องการเงิน

สไตล์การพูด — ฟังเหมือนเพื่อนนักลงทุนคุยกัน ไม่เหมือนบทความวิจัย
- สั้น ตรง 1-3 ประโยคพอ ไม่ขึ้นต้นทุกประโยคด้วย "พอร์ตของคุณ..." หรือ "ผู้ใช้..."
- ใช้คำที่นักลงทุนไทยใช้จริง: ติดดอย, DCA, ทำกำไร, ตัดขาดทุน, ทยอยเข้า, ขายล็อกกำไร, ถือยาว, กระจุกตัว, ลดความเสี่ยง, ดีดกลับ, ราคาแพง
- หลีกเลี่ยงภาษาเขียนทางการ: "พึงพิจารณา", "ดำเนินการ", "ในการนี้", "ขอแนะนำให้", "อย่างใกล้ชิด"

ตัวอย่างที่ดี vs ไม่ดี:
❌ "ขอแนะนำให้ผู้ใช้ดำเนินการพิจารณาลดสัดส่วนการถือครองหุ้น PTT"
✓ "PTT น้ำหนัก 38% — น่าทยอยลดบ้าง"

❌ "พอร์ตของคุณมีลักษณะของการกระจุกตัวในกลุ่มพลังงาน"
✓ "พลังงานหนัก 60% ติดดอยกลุ่มเดียวอาจเหนื่อย"

❌ "ขอแนะนำให้ติดตามผลประกอบการอย่างใกล้ชิด"
✓ "งบ Q2 ใกล้ออก ดูเลขก่อนตัดสินใจ"

กฎความปลอดภัย:
- ห้ามสั่ง "ต้องซื้อ/ขาย" ตรงๆ — ใช้ "น่าทยอย…", "ลองลด…", "ติดตามก่อน"
- ตัวเลขใช้เลขอารบิก พันคั่นด้วย ,
- ห้ามใส่ markdown ที่ LINE ไม่รองรับ (ตาราง, header) — bullet ใช้ "•"`;

// Three tiers, each overridable per environment:
//   FAST     — chat, vision, holdings status, daily picks/news (latency- and cost-sensitive)
//   BALANCED — portfolio analysis + comparison (quality matters, on-demand)
//   DEEP     — portfolio rebalance only (recommendation-heavy, used rarely)
export function modelFast(env)     { return env.CLAUDE_MODEL_FAST     || env.CLAUDE_MODEL || 'claude-haiku-4-5'; }
export function modelBalanced(env) { return env.CLAUDE_MODEL_BALANCED || 'claude-sonnet-4-6'; }
export function modelDeep(env)     { return env.CLAUDE_MODEL_DEEP     || 'claude-opus-4-7'; }

export async function askClaude(messages, env, opts = {}) {
  const model = opts.model || modelFast(env);
  const body = {
    model,
  ...
  // Adaptive thinking is opt-in per call. Opus 4.7 rejects the legacy
  // {type:"enabled", budget_tokens:N} shape; only {type:"adaptive"} works.
  ...
  const prompt = [
    {
      ...
      content:
        ...
        '\n' +
        'สไตล์ภาษาสำหรับ verdict_reason / observations / watch:\n' +
        '- พูดเหมือนเพื่อนนักลงทุนคุยกัน ไม่ใช่บทความวิจัย — 1 ประโยคต่อ 1 ข้อสังเกต\n' +
        '- ขึ้นต้นด้วย symbol/sector/ตัวเลข ไม่ใช่ "พอร์ตของคุณ..."\n' +
        '  ✓ "PTT น้ำหนัก 38% เกินเกณฑ์ — น่าทยอยลด"\n' +
        '  ✗ "พอร์ตของคุณมีการถือครอง PTT ในสัดส่วนที่ค่อนข้างสูง"\n' +
        '- ใช้คำที่นักลงทุนไทยใช้จริง: ติดดอย, กระจุกตัว, ราคาแพง, DCA, ทยอยเข้า, ขายล็อกกำไร\n' +
        '- หลีกเลี่ยง: พึงพิจารณา, ดำเนินการ, ขอแนะนำ, อย่างใกล้ชิด',
    },
  ];
  ...
  const prompt = [
    {
      ...
      content:
        ...
        '- ห้ามแนะนำ "ซื้อ/ขาย" ตรงๆ — ใช้ "น่าทยอย…", "ลองลด…", "ติดตามก่อน"\n' +
        '- ทุกตัวที่อยู่ใน list ต้องมี item ตอบกลับ (ถ้า no_realtime_data ให้ Hold/Watch + rationale ว่าไม่มีข้อมูลราคา)\n' +
        '\n' +
        'สไตล์ rationale: 1 ประโยคสั้น ขึ้นต้นด้วยตัวเลข/บริบท ไม่ใช่ "ควรพิจารณา..."\n' +
        '  ✓ "วันนี้ -3.2% หลุดต่ำสุดในรอบเดือน น่าระวัง"\n' +
        '  ✓ "ใกล้ 52w-high กำไร 28% แล้ว ทยอยล็อกกำไรได้"\n' +
        '  ✗ "แนะนำให้พิจารณาดำเนินการลดสัดส่วน เนื่องจากราคาปรับลดลง"\n' +
        '- ตอบสั้น 1500 tokens',
    },
  ];
  ...
  const prompt = [
    {
      ...
      content:
        ...
        '\n' +
        'สไตล์ summary / note / insights:\n' +
        '- พูดเหมือนเพื่อนเทียบพอร์ตให้ฟัง 1 ประโยคต่อข้อ ไม่ใช่บทวิเคราะห์\n' +
        '  ✓ "A หนักพลังงาน 60% — B กระจายกลุ่มมากกว่า"\n' +
        '  ✗ "พอร์ตทั้งสองมีลักษณะการกระจายความเสี่ยงที่แตกต่างกัน"',
    },
  ];
  const raw = await askClaude(prompt, env, {
    model: modelBalanced(env),
  ...
  const prompt = [
    {
      ...
      content:
        ...
        '- risk_notes 1-2 ข้อ\n' +
        '- ห้ามพูด "ต้องซื้อ/ขาย"\n' +
        '- ตัวเลข weight เป็น number ไม่ใช่ string\n' +
        '- ตอบให้สั้นกระชับ ไม่เกิน 1500 tokens\n' +
        '\n' +
        'สไตล์ภาษาสำหรับ summary / rationale / reason / risk_notes:\n' +
        '- พูดเหมือนเพื่อนนักลงทุนคุยกัน ไม่ใช่บทความวิจัย — 1 ประโยคต่อข้อ\n' +
        '- ห้ามขึ้นต้นด้วย "ควรพิจารณา…" / "ขอแนะนำให้…" / "ผู้ใช้ควร…"\n' +
        '- ห้าม copy action enum มาเป็นคำในประโยค reason (เช่น action=Trim แต่ reason ไม่ต้องเริ่มด้วย "ลดน้ำหนัก…")\n' +
        '- ใช้คำของนักลงทุนไทย: ติดดอย, ทยอยเข้า, ขายล็อกกำไร, DCA, กระจุกตัว, ราคาแพง\n' +
        '\n' +
        'ตัวอย่าง reason ที่ดี vs ไม่ดี:\n' +
        '  ✓ "น้ำหนัก 38% เกินครึ่งของพอร์ตกลุ่มแบงก์ — ขายล็อกกำไรบางส่วน"\n' +
        '  ✓ "ราคาขึ้นไป 28% แล้ว ใกล้ 52w-high แบ่งทำกำไรได้"\n' +
        '  ✓ "ติดลบ 15% แต่ปัจจัยพื้นฐานยังโอเค — DCA เพิ่มได้"\n' +
        '  ✗ "แนะนำให้ดำเนินการลดน้ำหนักหุ้นในกลุ่มธนาคารอย่างค่อยเป็นค่อยไป"',
    },
  ];
  // Rebalance is the one true recommendation task — uses Opus 4.7 with
  // adaptive thinking and effort:"high". max_tokens is generous because
  // thinking tokens count toward output; final JSON is still small.
  const raw = await askClaude(prompt, env, {
    model: modelDeep(env),
    maxTokens: 16000,
  ...
  const prompt = [
    {
      ...
      content:
        ...
        '  ✓ "PTT รับอานิสงส์น้ำมันดิบ Brent ทะลุ 90"\n' +
