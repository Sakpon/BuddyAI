...
import { backfillAssetClasses, getNetWorth, reclassifyHoldings, tagSymbolClass } from './wealth.js';
...
async function handleGoalWizardAnswer(ev, env, userId, text, state) {
  ...
  if (state.step === 'horizon') {
    ...
    return reply(env, ev.replyToken, actionAckCard({
      ...
      lines: [
        { text: '• พิมพ์ "default" สำหรับ 50/15/25/10 (หุ้นไทย / หุ้นสหรัฐ / ETF ตปท / เงินสด)', color: '#475569' },
        { text: '• หรือพิมพ์เอง 4 ตัวเลขรวม 100 เช่น "40 25 25 10"', color: '#475569' },
        { text: '• แบบเดิม 3 ตัวเลข (ไทย / ETF / เงินสด) ก็ใช้ได้', color: '#94A3B8' },
      ],
    }));
  }
...
// One-shot per-user backfill — re-runs inferAssetClass on holdings still
...
async function ensureHoldingsClassified(env, userId) {
  ...
  if (changed > 0) await logEvent(env, userId, 'holdings_reclassified', { changed });
}
...
async function showGoal(ev, env, userId) {
  ...
  await ensureHoldingsClassified(env, userId).catch(() => {});
...
const EDIT_FIELD_PROMPTS = {
  ...
  allocation: {
    ...
    subtitle: 'พิมพ์ 4 ตัวเลขรวม 100 (หุ้นไทย / หุ้นสหรัฐ / ETF ตปท / เงินสด) เช่น "40 25 25 10" — แบบเดิม 3 ตัว (ไทย/ETF/เงินสด) ก็ใช้ได้',
  },
};
