...
ถ้าเป็น (A):
{
  ...
  "holdings": [
    {
      "symbol": "<ตัวย่อ เช่น PTT, AOT, KBANK, SCBGOLD, AAPL, NVDA, BABA>",
      "asset_class": "<thai_equity | us_equity | global_etf | hk_equity | thai_fund | cash | crypto | other>",
...
asset_class: thai_equity = SET/MAI (PTT, AOT, KBANK), us_equity = หุ้นสหรัฐรายตัว (AAPL, NVDA, BABA, NIO, COIN, PLTR …), global_etf = ETF/Index Fund (SPY, VOO, QQQ, VTI, ARKK), hk_equity = หุ้นฮ่องกง (เลข 4-5 หลัก), thai_fund = กองทุนรวมไทย (KFUSA, B-INNOTECH, SCBGOLD).
...
รูปแบบที่รองรับ:
(1) Slip / ใบยืนยันเดียว — มียอดเดียวชัดเจน → ใส่ amount_thb เป็นยอดนั้น
(2) หน้า Activity / Transaction History ที่มีหลายรายการ Buy/Sell —
    ให้รวมยอด "Buy" ทุกรายการที่เห็น (ข้าม Sell, ค่าธรรมเนียม, เงินฝาก/ถอน)
    แปลงสกุลเงินอื่นเป็นบาทถ้าจำเป็น (ใช้อัตราโดยประมาณ USD≈36 / HKD≈4.6) แล้วใส่ใน amount_thb
    asset_class + symbol = ของรายการ Buy ที่ใหญ่ที่สุด (dominant)
    ใส่จำนวนรายการ Buy ที่เห็นใน warnings เพื่อให้ผู้ใช้ตรวจสอบ เช่น "รวม Buy 3 รายการ"
...
- "us_equity"    → หุ้นบริษัทสหรัฐรายตัว (AAPL, MSFT, GOOGL, AMZN, META, TSLA, NVDA, TSM, BABA, NIO, COIN ฯลฯ)
- "global_etf"   → ETF / Index Fund (SPY, VOO, QQQ, VTI, VT, IVV, SCHD, GRID, LIT, ARKK, GLD ฯลฯ)
...
ตอบเป็น JSON เพียงอย่างเดียว:
{
  ...
  "asset_class": "thai_equity | us_equity | global_etf | hk_equity | thai_fund | cash | crypto | other | null",
  "symbol": "<ticker หรือชื่อกองทุนถ้าเห็นชัด เช่น PTT / SPY / KFUSA / 0700 ไม่งั้น null>",
  "description": "<1 วลีสั้นๆ บอกว่าเป็นรายการอะไร เช่น 'ซื้อกองทุน SCBGOLD' / 'รวมยอดซื้อ 3 รายการ'>",
  "warnings": [<คำเตือนถ้าตัวเลขไม่ครบหรือไม่ชัด หรือกรณีรวมหลายรายการ>]
}
